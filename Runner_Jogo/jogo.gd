...
func _process(delta: float) -> void:
	...
	$MeioAzul.position.x -= velocidade * delta*0.08
	$MeioAzul2.position.x -= velocidade * delta*0.08
	$MeioPreto.position.x -= velocidade * delta*0.08
	$MeioPreto2.position.x -= velocidade * delta*0.08
	...
	if $Chao.position.x < -1152: # se o chão sair de cena pela esquerda
